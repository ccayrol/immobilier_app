...
function About() {
  ...
  const nextSlide = () => setCurrentSlide((prev) => (prev + 1) % bureauImages.length);
  const prevSlide = () => setCurrentSlide((prev) => (prev - 1 + bureauImages.length) % bureauImages.length);
  const goToSlide = (index) => setCurrentSlide(index);
  const handleContactClick = () => navigate('/sendmail');

  const agencyAddress = "54 rue de Seguey, 33000 Bordeaux";
  const agencyEmail = "partenaireinvestissement@gmail.com";
  const agencyPhone = "+33 5 24 60 24 60";
    ...
    <div className="min-h-screen w-full bg-white">
      <div className="px-6 sm:px-12 py-12">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-center mb-16">
          ...
          <p className="text-lg sm:text-xl text-gray-700 max-w-3xl mx-auto px-4 mb-4">
          ...
          <p className="text-base sm:text-lg text-gray-600 max-w-2xl mx-auto">
            Une activité solide et pérenne depuis 1996.{' '}
            <a href="https://www.pappers.fr/entreprise/partenaire-investissement-403518509" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
              Vérifiez nos informations sur Pappers
            </a>.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="mb-16 px-4">
          ...
          <p className="text-lg text-gray-700 leading-relaxed">
            Fondée en 1996 par Eric Soubeste, notre agence s'est construite sur des valeurs fortes : l'excellence, la confiance et la proximité avec nos clients. Grâce à notre expertise, nous sélectionnons les meilleures opportunités immobilières à Bordeaux.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }} className="mb-16 px-4">
          <h2 className="text-3xl font-bold text-black mb-6 text-center">Notre Bureau</h2>
            ...
            <div className="p-8 bg-gray-50 rounded-2xl shadow-xl">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-10">
                  <div>
                    <h3 className="text-2xl font-semibold flex items-center text-black mb-3">
                      <Clock className="mr-2 h-5 w-5 text-gray-700" /> Horaires
                    </h3>
                    <div className="grid grid-cols-2 sm:grid-cols-1 gap-2 text-gray-700 text-sm">
                ...
                <div className="space-y-6">
                  <div className="w-full h-[300px] lg:h-[350px] border border-gray-200 rounded-xl shadow overflow-hidden">
                    <iframe title="Localisation de l'agence" src={googleMapsUrl} className="w-full h-full" frameBorder="0" allowFullScreen></iframe>
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-2xl font-semibold flex items-center text-black">
                      <MapPin className="mr-2 h-5 w-5 text-gray-700" /> Adresse
                    </h3>
                    <p className="text-gray-700 text-lg">{agencyAddress}</p>
                    <button onClick={() => window.open(`https://www.google.com/maps/dir//${encodedAddress}`, '_blank')} className="mt-2 px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 text-sm">
                      Voir l'itinéraire
                    </button>
                  </div>
                </div>
